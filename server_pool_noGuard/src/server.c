	...
	{	
		...
		for(i = 0; i < ready_events; ++i)
		{
			...
			if(fd == fd_listen){
				...
				for(index = 0; index < nchild; ++index)
				{
					if(read(parr[index].child_fd, flag, sizeof(flag)) >0 ){/* how to judge the child is dead?????*/
						make_child(parr, nchild, index, nchild-1);		
					}
				}
